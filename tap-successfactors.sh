...
cd "$TOML_DIR" || exit
poetry install 1>&2
# poetry run tap-successfactors --config=config.json $*
# poetry run tap-successfactors --config=config.json $* > output/output.json
poetry run tap-successfactors --config=config.json --state=state.json $* > output/output.json